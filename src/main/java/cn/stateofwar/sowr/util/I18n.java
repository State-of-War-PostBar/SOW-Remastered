...
public class I18n {
  ...
  public static void init() {
    try {
      for (Locale l : Locale.values()) l.readValues();
    } catch (IOException e) {
  ...
  public static enum Locale {
     ...
     *
     * @param key Translation key.
     * @return Translation result. If no translation available, the key itself will be returned.
     */
     ...
     * @return The locale parsed.If the name is not valid, default locale <i>(English - United
     *     States)</i> will be selected.
     */
    public static Locale parseLocale(String _name) {
      for (Locale locale : Locale.values()) if (locale.getName().equals(_name)) return locale;
      return EN_US;
    }
  }
