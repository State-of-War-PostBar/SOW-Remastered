...
public class Utils {
  ...
  public static OSType getOS() {
    ...
    if (os_name.indexOf("nux") >= 0) return OSType.LINUX;
    else if (os_name.indexOf("win") >= 0) return OSType.WINDOWS;
    else if ((os_name.indexOf("mac") >= 0) || (os_name.indexOf("darwin") >= 0)) return OSType.MAC;
  ...
  public static List<String> getResourceAsStrings(String path) throws IOException {
    return Resources.readLines(
        Resources.getResource(path), Charset.forName(References.DEFAULT_TEXT_ENCODING));
  }
  ...
  public static String listToString(List<String> list, boolean new_line) {
    ...
    for (String s : list) sb.append(new_line ? s + nl() : s);
  ...
  public static String readLine(String path, int line) throws IOException {
    ...
    if (lines.size() < line) line = lines.size();
   ...
   * @param path Path of the text file.
   * @param line Line to read, starts with 1.
   * @param position Position to read, starts with 1. Text starts being read by this position, not
   *     after.
   * @return Text read.
   * @throws IOException
   */
  public static String readLine(String path, int line, int position) throws IOException {
    ...
    if (l.length() < position) position = 1;

    return l.substring(position, l.length());
  }

  /**
   * Read a text file to a single string, with line separator characters and spaces.
   *
   * @param path Path of the text file.
   * @return Text read.
   * @throws IOException
  ...
  public static String readFileToString(String path) throws IOException {
    return Files.asCharSource(new File(path), Charset.forName(References.DEFAULT_TEXT_ENCODING))
        .read();
  }

  /**
   * Write a line to the end of the text file.<br>
   * <i>Will set a line separator at the end, but will not automatically start writing at a new line
   * in the beginning!</i>
   *
   * @param path Path of the text file.
  ...
  public static void writeLine(String path, String text, int line) throws IOException {
    ...
    java.nio.file.Files.write(
        file.toPath(), lines, Charset.forName(References.DEFAULT_TEXT_ENCODING));
  }
   ...
   * @param text Text to write.
   * @param line Line to write, starts with 1.
   * @param position Position to write, starts with 1. Text starts being replaced by this position,
   *     not after.
   * @throws IOException
   */
  public static void writeLine(String path, String text, int line, int position)
      throws IOException {
    position--;
    line--;
    File file = new File(path);
